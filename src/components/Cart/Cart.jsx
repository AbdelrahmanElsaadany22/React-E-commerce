...
export default function Cart() {
  let { getLoggedUserCart, updateCartItemCount, deleteProductItem, setCart } = useContext(CartContext);
  ...
  async function updateCartCount(productId, count) {
    let res = await updateCartItemCount(productId, count);
    setCartDetails(res.data.data);
  }

  async function deleteItem(productId) {
    let res = await deleteProductItem(productId);
    setCartDetails(res.data.data);
    setCart(res.data);
  }
  ...
  return (
    <>
      <div className="relative overflow-x-auto sm:rounded-lg">
        ...
        <h3 className="text-center text-slate-600 text-lg font-light">
        ...
        <Link to={'/checkout'}>
          <button className="btn w-full bg-green-600 text-white">Check Out</button>
        </Link>
      </div>
    </>
  );
